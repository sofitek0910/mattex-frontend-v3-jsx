...
import { StepsFormProvide } from '@ant-design/pro-form/lib/layouts/StepsForm';
...
const DNDPanel = ({
  ...
  const addAllHandlers = () => {
    const dataSource = sideBarList.map((item, index) => ({
      id: uuidv4(),
      type: item.type,
      editing: false,
      data: { payload: stateList[item.type] },
    }));
    setRootDataSource([...rootDataSource, ...dataSource]);
    const sideArr = [...sideBarList].map((x) => ({ ...x, isUse: !x.inUse }));
  ...
  const SortableContainer = sortableContainer(({ children }) => {
    return <ul style={{ padding: '2px' }}>{children}</ul>;
  });
  ...
  return (
    <div className="DNDPanel">
      <Row>
        ...
        <Col flex="auto" style={{ maxWidth: '100%' }}>
