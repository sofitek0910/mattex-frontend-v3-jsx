...
const ImageComponent = ({
  ...
  const editLabelHandler = () => {
    ...
    changeHandler(index, newLabel, prefilledValue);
  ...
  return (
    ...
    >
      <Col flex={editing && canEditLabel ? '180px' : '200px'}>
        { editing && canEditLabel && labelEditing ?(
      ...
      {editing && canEditLabel ? (
        <Col flex="20px" style={{ padding: '0px' }}>
          <Button type="text" style={{ padding: '0px' }}>
            {labelEditing ? (
              <CheckCircleOutlined style={{ color: '#0256B4' }} onClick={() => editLabelHandler()}/>
            ) : (
              <EditOutlined style={{ color: '#0256B4' }} onClick={() => setLabelEditing(true)}/>
            )
            }
          </Button>
        </Col>
